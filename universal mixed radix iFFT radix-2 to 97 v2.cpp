...
#ifdef NO_PAUSE
#define APP_PAUSE() ((void)0)
#else
#define APP_PAUSE() system("pause")
#endif
...
/*
    ...
    cout<<endl;
    cout<<endl;
    cout<<"if radix-4 == DFT tab2[j].real(): "<<endl;APP_PAUSE();
*/
       for(int j=0;j<N;j++)
        {
            if((j-1)%10000==0){APP_PAUSE()  ;}
            ...
            for(int i=0;i<N;i++)
        {
         ...
         if((fabs(tab3[i].real()-tab2[j].real())<=0.03)&&(fabs(tab3[i].imag()-tab2[j].imag())<=0.03))
          {
              if(i==30){   cout<<" ...   DFT["<<i<<"]==FFT["<<j<<"]= "<<round(tab2[j].real()*1000)/1000<<"  "<<endl;APP_PAUSE();}
            cout.precision(4);
